...
const ENDPOINTS = [
  ...
  ["Naver (Korea)", "https://searchadvisor.naver.com/indexnow"],
  ["Seznam (Czech)", "https://search.seznam.cz/indexnow"],
  ["Yep (Ahrefs)", "https://indexnow.yep.com/indexnow"],
...
async function submitTo(endpoint, name) {
  let ok = 0, fail = 0
  for (let i = 0; i < urls.length; i += 50) {
    ...
    try {
      ...
      if (r.status === 200 || r.status === 202) { ok += batch.length } 
      else { fail += batch.length; process.stdout.write("x") }
    } catch(e) { fail += batch.length; process.stdout.write("!") }
    process.stdout.write(".")
  }
  return { name, ok, fail, total: urls.length }
}

async function main() {
  const results = await Promise.all(ENDPOINTS.map(([name, url]) => submitTo(url, name)))
  console.log()
  for (const r of results) {
    console.log(r.name.padEnd(18), r.ok+"/"+r.total, r.fail > 0 ? "("+r.fail+" failed)" : "")
  }
}
