...
function AnimatedCounter({ target, format, duration = 2000 }: { target: number; format: (n: number) => string; duration?: number }) {
      ...
      <main className="flex-1">
        ...
        <section className="relative overflow-hidden section-padding bg-warm">
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div className="absolute -top-40 -right-40 w-[800px] h-[800px] bg-gradient-to-br from-primary/8 via-accent/5 to-transparent rounded-full blur-[120px] floating" />
            <div className="absolute -bottom-40 -left-40 w-[700px] h-[700px] bg-gradient-to-tr from-accent/8 via-primary/5 to-transparent rounded-full blur-[120px] floating-delayed" />
            ...
            <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
              <div className="reveal">
                <div className="flex items-center gap-3 mb-6">
                  <Badge className="px-4 py-1.5 text-sm bg-primary/10 text-foreground border-primary/20 rounded-full font-medium">
                    {t("homepage.badge")}
                  </Badge>
                ...
                <div className="glass-card p-1.5 reveal reveal-delay-1 max-w-xl">
                  ...
                  {tab === "shorten" ? (
                    <>
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        />
                        <Button onClick={handleShorten} disabled={loading} className="h-12 px-6 w-full sm:w-auto bg-gradient-to-r from-primary to-accent text-white hover:from-primary/90 hover:to-accent/90 shadow-glow">
                    ...
                    <>
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Input
                          ...
                          className="flex-1 h-12 text-sm"
                        />
                        <Button onClick={handleGenerateQR} className="h-12 px-6 w-full sm:w-auto bg-gradient-to-r from-primary to-accent text-white hover:from-primary/90 hover:to-accent/90 shadow-glow">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-foreground/50 reveal reveal-delay-2">
                  <span className="inline-flex items-center gap-1.5">
                    <Star className="w-4 h-4 text-accent fill-accent" />
                    {t("homepage.trustedBy")}
                  </span>
                  <Link href="/browser-extension" className="inline-flex items-center gap-1.5 text-accent hover:text-accent/80 transition-colors font-medium">
              ...
              <div className="hidden lg:block reveal reveal-delay-2">
                <div className="relative">
                  ...
                  <div className="relative glass rounded-2xl p-6 shadow-strong space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="glass rounded-xl p-4">
                        <p className="text-xs text-foreground/40 mb-1">{t("homepage.totalClicks")}</p>
                        <p className="text-2xl font-bold"><AnimatedCounter target={12500} format={(n) => (n / 1000).toFixed(1) + "K"} /></p>
                        <div className="flex items-center gap-1 mt-1">
                          <TrendingUp className="w-3 h-3 text-green-500" />
                          <span className="text-xs text-green-500 font-medium">+23.5%</span>
                        </div>
                      </div>
                      <div className="glass rounded-xl p-4">
                        <p className="text-xs text-foreground/40 mb-1">{t("homepage.topCountry")}</p>
                        <p className="text-2xl font-bold">US</p>
                        <p className="text-xs text-foreground/40 mt-1">34.2% of traffic</p>
                      </div>
                      <div className="glass rounded-xl p-4">
                        <p className="text-xs text-foreground/40 mb-1">{t("homepage.topDevice")}</p>
                        <p className="text-2xl font-bold">65%</p>
                        <p className="text-xs text-foreground/40 mt-1">{t("homepage.mobile")}</p>
                      </div>
                      <div className="glass rounded-xl p-4">
                        <p className="text-xs text-foreground/40 mb-1">{t("homepage.avgCtr")}</p>
                    ...
                    <div className="glass rounded-xl p-4">
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-xs text-foreground/40 font-medium">{t("homepage.clicksLast7Days")}</p>
                        <span className="text-xs text-green-500 font-medium">+12.3%</span>
                      </div>
                      <div className="flex items-end gap-2 h-16">
                        {["mon","tue","wed","thu","fri","sat","sun"].map((day, i) => (
                          <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            <div
                              className="w-full rounded-md bg-gradient-to-t from-primary to-accent/70 transition-all duration-500"
                    ...
                    <div className="flex items-center justify-center gap-4 text-xs text-foreground/50">
                      <span className="inline-flex items-center gap-1">
                        <MousePointerClick className="w-3.5 h-3.5" /> <LiveTicker /> clicks today
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <Link2 className="w-3.5 h-3.5" /> <AnimatedCounter target={2400} format={(n) => (n / 1000).toFixed(1) + "K"} /> links
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <Smartphone className="w-3.5 h-3.5" /> 65% mobile
                      </span>
                      <span className="inline-flex items-center gap-1">
                        <Zap className="w-3.5 h-3.5" /> <AnimatedCounter target={99.9} format={(n) => n.toFixed(1) + "%"} /> uptime
                      </span>
                    </div>
                  </div>
                </div>
